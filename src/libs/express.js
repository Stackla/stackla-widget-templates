...
const { getAndRenderTiles, getTilesToRender, renderTemplates } = require("./tile.handlers");
const widgetOptions = require("../../tests/fixtures/widget.options");
const cookieParser = require("cookie-parser");
const tiles = require("../../tests/fixtures/tiles");
const { createMockRoutes, STAGING_UI_URL } = require("../../tests/libs/developer");
const fs = require("fs");
const apicache = require("apicache");

function getDomain(env = process.env.APP_ENV) {
...
async function getContent(widgetType) {
...
async function getHTML(content, request) {
...
function mutateStylesForCustomWidgets(widgetType) {
...
expressApp.post("/development/widgets/:wid/draft", async (req, res) => {
  ...
  const widgetOptionsMutated = mutateStylesForCustomWidgets(req.cookies.widgetType);
...
expressApp.get("/development/widgets/:wid/draft", async (req, res) => {
  const widgetType = req.query.widgetType || req.cookies.widgetType;
...
expressApp.get("/development/widgets/:wid/rendered/tiles", async (req, res) => {
  const content = await getContent(req.query.widgetType);
...
expressApp.get("/preview", async (req, res) => {
  ...
  res.render("preview", {
    ...
    ...(await getContent(req.query.widgetType))
  });
});

expressApp.get("/multi-preview", async (req, res) => {
  const widgetRequest = req.query;
  const widgetType = req.query.widgetType;
...
module.exports.determineEnvironment = determineEnvironment;
